...
await check("route contract: actions hard-deletes strain + reverses rep + busts strains only", () => {
  ...
  assert.ok(src.includes('reverseReputationBySource("STRAIN", targetId'), "STRAIN_CREATED reversed")
  assert.ok(src.includes('reverseReputationBySource("STRAIN_PHOTO", pid'), "STRAIN_PHOTO reversed per photo")
