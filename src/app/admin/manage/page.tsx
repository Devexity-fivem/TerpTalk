...
import AdminTerpBot from "@/components/admin-terpbot"
...
export default function AdminPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  // Deep links like /admin/manage?tab=feedback land on the right surface.
  const initialTab = use(searchParams).tab
  const { data: session, status } = useSession()
  const router = useRouter()
  const role = (session?.user as { role?: string })?.role

  const [tab, setTab] = useState<string>(
    initialTab && (TABS as readonly { id: string }[]).some((x) => x.id === initialTab) ? initialTab : "overview",
  )
