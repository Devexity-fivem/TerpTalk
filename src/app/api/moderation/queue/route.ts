  ...
  if (kind === "ALL" || kind === "REPORT") {
    ...
    for (const r of reports) {
      ...
      items.push({
        ...
        assignedTo: r.assignedToId ? names.get(r.assignedToId) ?? "Deleted user" : null,
        type: r.type,
        reason: r.reason,
  ...
  if (kind === "ALL" || kind === "FLAG") {
    ...
    for (const f of flags) {
      items.push({
        ...
        assignedTo: f.assignedToId ? names.get(f.assignedToId) ?? "Deleted user" : null,
        signal: f.signal,
        signalLabel: SIGNAL_LABELS[f.signal] ?? f.signal,
        subject: names.get(f.userId) ?? "Deleted user",
        subjectId: f.userId,
        counterparty: f.counterpartyId ? names.get(f.counterpartyId) ?? "Deleted user" : null,
        evidence: f.evidence,
      })
    }
  }
