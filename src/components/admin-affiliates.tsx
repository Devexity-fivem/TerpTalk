            ...
            <div key={p.id} className="flex items-center justify-between gap-3 border border-border rounded-lg p-3">
              ...
              <div className="flex gap-1.5 shrink-0">
                ...
                <Tooltip content="Delete partner">
                  <button onClick={() => confirm(`Delete ${p.name} and all its products?`) && fetch(`/api/admin/affiliates/partners?id=${p.id}`, { method: "DELETE" }).then(load)} className="p-1.5 rounded hover:bg-destructive/15 text-destructive" aria-label={`Delete ${p.name}`}><Trash2 className="w-4 h-4" /></button>
                </Tooltip>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-3 mb-5">
          {products.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-3 border border-border rounded-lg p-3">
              ...
              <div className="flex gap-1.5 shrink-0">
                <Tooltip content="Test affiliate link">
                  <a href={`/go/${p.slug}`} target="_blank" rel="noreferrer" className="p-1.5 rounded hover:bg-secondary" aria-label={`Test affiliate link for ${p.name}`}><ExternalLink className="w-4 h-4" /></a>
                </Tooltip>
                ...
                <Tooltip content="Delete product">
                  <button onClick={() => confirm(`Delete ${p.name}?`) && fetch(`/api/admin/affiliates/products?id=${p.id}`, { method: "DELETE" }).then(load)} className="p-1.5 rounded hover:bg-destructive/15 text-destructive" aria-label={`Delete ${p.name}`}><Trash2 className="w-4 h-4" /></button>
                </Tooltip>
              </div>
            </div>
          ))}
